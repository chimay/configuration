...
#         2. vim ~/.w3m/keymap
#               keymap  d      COMMAND     "EXTERN 'echo %s >> ~/.w3m/restore-tab.txt' ; CLOSE_TAB"
#               keymap  u      TAB_GOTO    /usr/lib/w3m/cgi-bin/restore_tab.cgi

#Open the last closed tab
last_tab=$(tail -n 1 ~/.w3m/restore-tab.txt);
#limit of tabs stored
limit=$(tail -n 300 ~/.w3m/restore-tab.txt);
other_tabs=$(printf "%s" "$limit" | head -n -1);
printf "%s\r\n" "$other_tabs" > ~/.w3m/restore-tab.txt;
